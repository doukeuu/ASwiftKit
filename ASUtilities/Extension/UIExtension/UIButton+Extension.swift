...
extension UIButton {
    ...
    func resetImageTitlePosition(_ position: ButtonImageTitlePosition, space: CGFloat) {
        ...
        if (UIDevice.current.systemVersion as NSString).floatValue >= 8.0 {
            ...
            titleWidth = min((self.bounds.width - imageWith), (titleLabel?.intrinsicContentSize.width ?? 0.0))
            titleHeight = min((self.bounds.height - imageHeight), (titleLabel?.intrinsicContentSize.height ?? 0.0))
        } else {
            titleWidth = min((self.bounds.width - imageWith), (titleLabel?.frame.size.width ?? 0.0))
            titleHeight = min((self.bounds.height - imageHeight), (titleLabel?.frame.size.height ?? 0.0))
        }
